  ...
  subroutine deallocate_main( option )
    ...
    case( '1e_int' )
       ...
       if( allocated(dipzmoa) ) then
          write(iout,'(A)') ' DEALLOCATE dipzmoa, dipymoa, dipxmoa '
          flush(iout)
...
!:          call track_mem( -nrorb*nrorb )
       ...
       if( allocated(socmoAB) ) then
          write(iout,*) " deallocating socmoAB,socmoBB,socmoAAsocmoAB,socmoBB,socmoAA"
  ...
  subroutine writeme_modreadin(myroutine,option)
    ...
    case('read_input')
       ...
       case( 'greeting' ) 
          ...
          write(iout,'(A)') divide
       case( 'date' )          
          write(iout,'(A)') ' I WAS COMPILED ON Sun Dec 10 00:09:29 EST 2023 '
